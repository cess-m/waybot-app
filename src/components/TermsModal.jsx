...
const TermsModal = ({ isOpen, onAccept, onDecline, studentName }) => {
  ...
  const isLocked = !hasScrolledToEnd;
      ...
      <div className="relative w-full max-w-[95vw] sm:max-w-2xl md:max-w-3xl 
          ...
          <div className="space-y-5 sm:space-y-6 md:space-y-7 text-slate-300">
            ...
            <div className="relative pl-4 sm:pl-6 border-l-2 border-violet-500/30">
              ...
              <p className="text-xs sm:text-sm leading-relaxed text-slate-400">
                These Terms constitute a legally binding agreement between you (the Student User) and the WayBot platform. WayBot is owned by <span className="font-semibold text-slate-300">Khatlyn Mae Parages</span> and is developed and maintained by <span className="font-semibold text-slate-300">Princess Mae Parages</span> as the Main Developer. Your continued use of the platform signifies your ongoing acceptance of these Terms and any future updates or amendments that may be implemented.
              </p>
            </div>
            ...
            <div className="relative pl-4 sm:pl-6 border-l-2 border-violet-500/30">
              ...
              <p className="text-xs sm:text-sm leading-relaxed text-slate-400 mb-2">
                All content generated by WayBot, including explanations, step-by-step solutions, and educational materials, is the intellectual property of the WayBot platform under the ownership of <span className="font-semibold text-slate-300">Khatlyn Mae Parages</span>. The platform is developed and maintained by <span className="font-semibold text-slate-300">Princess Mae Parages</span> as the Main Developer, unless otherwise stated.
              </p>
              <p className="text-xs sm:text-sm leading-relaxed text-slate-400">
        ...
        <div className="px-3 sm:px-6 md:px-8 py-3 bg-slate-900/50 border-t border-slate-700/30">
          <div className="space-y-3">
            ...
            <label
              className={`flex items-start gap-3 text-sm sm:text-base transition-opacity
                ${isLocked || isDeclining ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
            >
              <input
                type="checkbox"
                checked={checked1}
                disabled={isLocked || isDeclining}
                onChange={(e) => {
                  if (!isLocked && !isDeclining) setChecked1(e.target.checked);
                }}
            ...
            >
              <input
                ...
                disabled={isLocked || isDeclining}
                onChange={(e) => {
                  if (!isLocked && !isDeclining) setChecked2(e.target.checked);
                }}
                className="mt-1 w-4 h-4 text-violet-600 rounded focus:ring-violet-500 border border-slate-600 bg-slate-800 checked:bg-violet-600 checked:border-transparent"
              />
              <span className="text-slate-300">
        ...
        <div className="px-4 sm:px-6 md:px-8 py-4 sm:py-5 md:py-6 border-t border-slate-700/50 bg-gradient-to-t from-slate-800/50 to-transparent">
          ...
          <p className="text-center text-[10px] sm:text-xs text-slate-500 leading-relaxed px-1">
            {isLocked
              ? "Please scroll to the bottom (Section 7) to enable the consent options."
              : checked3
                ? "You selected to exit. Click 'Exit' to confirm."
                : "Both consent checkboxes must be selected to proceed."
            }
          </p>
        </div>
